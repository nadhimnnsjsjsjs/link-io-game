    ...
    socket.on('slicePlayer', (data) => {
        ...
        if (victim && attacker && victim.trail.length > 0) {
            ...
            if (slicePercent > 0.6 || (victim.score - stolen) < 100) {
                ...
                io.emit('syncScore', { playerId: attacker.id, newScore: attacker.score });
                let pos = victim.trail[data.sliceIndex];
                io.emit('doSlice', { victimId: data.victimId, sliceIndex: data.sliceIndex, x: pos.x, y: pos.y, stolen: stolen });
            }
        }
    });
...
http.listen(PORT, '0.0.0.0', () => console.log('Server Active: ' + PORT));
